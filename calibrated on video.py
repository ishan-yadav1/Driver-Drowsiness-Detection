...
# =============================================================================
# =============================================================================
# 5. NEW CALIBRATION FUNCTION (ADAPTED FOR VIDEO FILE)
# =============================================================================
def run_calibration():
    """
    Runs a calibration phase on a pre-recorded "Normal" video
    to determine a personalized EAR threshold for that subject.
    """
    # =========================================================================
    #
    #   >>> EDIT THIS LINE TO POINT TO YOUR "NORMAL" VIDEO FILE <<<
    #
    calibration_video_path = "17-MaleNoGlasses-Talking.avi"
    #
    # =========================================================================

    print(f"[INFO] Starting calibration using video: {calibration_video_path}")

    cap = cv2.VideoCapture(calibration_video_path)
    if not cap.isOpened():
        print(f"[ERROR] Could not open video file for calibration: {calibration_video_path}")
        return None
    ...
    while True:
        ...
        # We can optionally display the calibration video to see it happening
        cv2.putText(frame, "CALIBRATION IN PROGRESS...", (10, 30),
        ...
        # You can comment out the next two lines if you don't want to see the video
    ...
    if len(ear_values) == 0:
        print("[ERROR] Calibration failed. Could not detect open eyes in the video.")
        return None

    average_ear = sum(ear_values) / len(ear_values)
    print(f"[INFO] Average open-eye EAR for this subject is: {average_ear:.2f}")
...
def process_video_source(ear_threshold):
    ...
    cap = cv2.VideoCapture("17-MaleNoGlasses-Yawning.avi") # For video file
